...
<div class="wrap">
	...
	<div class="content">
		<form action="boardUpdate" method="post" class="write-form"
			...
			<input type="hidden" name="bnum" value="${reviewboard.rbnumber}">
			<input type="text" name="btitle" class="write-input"
				value="${reviewboard.rbtitle}" required>
			<textarea name="bcontents" rows="10" class="write-input ta">
				${reviewboard.rbtext}</textarea>
			<div class="filebox">
				<div class="befor-file" style="margin-bottom: 10px;">
					...
					<c:if test="${!empty rbfList}">
						...
						<label style="width: 100%;"><a href="./download?sysname=${file.rbf_sysname}">
							${file.rbf_oriname}</a></label>
						</c:forEach>
					</c:if>
				</div>
